  ...
  <div class="card mb-5 mb-xl-10">
    ...
    >
      ...
      <div class="card-title m-0">
        <h3 class="fw-bolder m-0">{{ translate("addAdministrator") }}</h3>
      </div>
      <!--end::Card title-->
    </div>
    <!--begin::Card header-->
...
export default defineComponent({
  ...
  setup() {
    ...
    const adminDetails = ref<AdminDetails>({
      name: '',
      account: '',
      status: 'Active',
      password: '',
    ...
    const saveChanges1 = () => {
      const uname = adminDetails.value.name;
      const account = adminDetails.value.account;
      const status = adminDetails.value.status;
      const password = adminDetails.value.password;
      const pickable = adminDetails.value.pickable;
      ...
      if (submitButton1.value) {
        ...
        axios
        .post("http://localhost:8000/admins/create", user, { headers: authHeader })
        .then((response) => { 
          Swal.fire({
            text: "Administrator create success.",
            icon: "success",
          ...
          Swal.fire({
            text: "Administrator create fail.",
            icon: "error",
            buttonsStyling: false,
    ...
    onMounted(() => {
      setCurrentPageBreadcrumbs("create", ["administrators"]);
    });
