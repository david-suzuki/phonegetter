  ...
  <div class="card mb-5 mb-xl-10">
    ...
    >
      ...
      <div class="card-title m-0">
        <h3 class="fw-bolder m-0">{{ translate("updateUser") }}</h3>
      </div>
      <!--end::Card title-->
    </div>
    <!--begin::Card header-->
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(() => {
      setCurrentPageBreadcrumbs("update", ["users"]);
    });
  ...
  mounted() {
    ...
    axios
    .get("http://localhost:8000/users/fetch/" + userId, { headers: authHeader })
    .then((response)=>
    { 
  ...
  methods: {
    saveChanges() {
      ...
      const userId = this.id;
      const userData = {id: this.id, user:this.userDetails};

      const token = localStorage.getItem('id_token');
      const authHeader = { Authorization: 'Bearer ' + token };
      
      axios
      .post("http://localhost:8000/users/update", userData, { headers: authHeader })
      .then((response)=>
      { 
        Swal.fire({
