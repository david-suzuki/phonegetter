...
export default defineComponent({
  ...
  setup() {
    ...
    const pageTitle = computed(() => {
      ...
      // When refreshing page, check if store data is loaded completely, fixing bug.
      if (pageTitle)
        return translate(pageTitle);
      return pageTitle;
    });

    const breadcrumbs = computed(() => {
      const pageBreadcrumbs = store.getters.pageBreadcrumbPath;
      // When refreshing page, check if store data is loaded completely, fixing bug.
      if (pageBreadcrumbs) {
        const translatedBreadcrumbs: string[] = [];
        for(let i = 0; i < pageBreadcrumbs.length; i++) {
          translatedBreadcrumbs.push(translate(pageBreadcrumbs[i]));
        }
